...
class Args:
    ...
    calvin_config_path: str = "/inspire/qb-ilm2/project/26summer-camp-10/public/two/calvin/calvin_models/conf"
...
class CalvinPolicyClient:
    ...
    def __init__(
        ...
        self.step_count = 0
        self.send_state = os.environ.get("CALVIN_SEND_STATE", "1").lower() in {"1", "true", "yes", "on"}
        self.state_dim = int(os.environ.get("CALVIN_STATE_DIM", "8"))
    ...
    @staticmethod
    def compute_task_progress(subtask: str, start_info: dict, current_info: dict) -> np.ndarray:
        if not subtask or start_info is None or current_info is None or not subtask.startswith("push_"):
        ...
        color = parts[1]
        direction = parts[-1]
        obj_name = f"block_{color}"
        direction_sign = 1.0 if direction == "right" else -1.0 if direction == "left" else 0.0
        threshold = 0.1
        ...
        try:
            ...
            current_pos = np.array(current_info["scene_info"]["movable_objects"][obj_name]["current_pos"])
        except KeyError:
            return np.zeros(5, dtype=np.float32)

        signed_disp = direction_sign * float(current_pos[0] - start_pos[0])
        progress = np.clip(signed_disp / threshold, -1.0, 2.0)
        remaining = np.clip((threshold - signed_disp) / threshold, -1.0, 2.0)
        return np.array([progress, remaining, signed_disp / threshold, threshold, direction_sign], dtype=np.float32)
    ...
    def step(
        ...
        if self.send_state:
            example["state"] = np.asarray(obs["robot_obs"][: self.state_dim], dtype=np.float32)[None, :]
        example["task_progress"] = self.compute_task_progress(subtask, start_info, current_info)
