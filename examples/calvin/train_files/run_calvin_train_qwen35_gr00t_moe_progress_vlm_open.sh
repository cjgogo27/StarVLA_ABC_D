...
# Qwen3.5 + GR00T-MoE-Progress post-train with VLM unfrozen.
# This is separate from run_calvin_train_qwen35_gr00t_moe_progress.sh,
# whose default freezes qwen_vl_interface.
...
Framework_name=QwenGR00TMoEProgress
freeze_module_list=''
base_vlm=/inspire/qb-ilm2/project/26summer-camp-10/public/two/Model/Qwen3.5-4B
config_yaml=./examples/calvin/train_files/starvla_train_calvin_gr00t_moe_progress.yaml
...
source_run_id=${SOURCE_RUN_ID:-qwen35vl4b_gr00t_calvin_abc_d_strong_aug}
run_id=${RUN_ID:-qwen35vl4b_gr00t_moe_progress_calvin_abc_d_vlm_open}
pretrained_checkpoint=${PRETRAINED_CHECKPOINT:-${run_root_dir}/${source_run_id}/checkpoints/steps_30000_pytorch_model.pt}
...
accelerate launch \
  ...
  --trainer.max_train_steps "${MAX_TRAIN_STEPS:-60000}" \
