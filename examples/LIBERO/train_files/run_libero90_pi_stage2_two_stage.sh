...
RUN_ID="${RUN_ID:-qwen35vl4b_pi_libero90_stage2_two_stage}"
STAGE1_CKPT="${STAGE1_CKPT:-./results/Checkpoints/qwen35vl4b_pi_libero90_stage1_head_only/checkpoints/steps_30000_pytorch_model.pt}"
...
if [[ ! -f "${STAGE1_CKPT}" ]]; then
  ...
  echo "Set STAGE1_CKPT=/path/to/steps_xxx_pytorch_model.pt" >&2
  exit 1
fi
...
accelerate launch \
  ...
  --trainer.learning_rate.base "${BASE_LR:-1.0e-06}" \
  --trainer.learning_rate.qwen_vl_interface "${QWEN_LR:-2.0e-06}" \
  --trainer.learning_rate.action_model "${ACTION_LR:-2.0e-05}" \
  --trainer.max_train_steps "${MAX_TRAIN_STEPS:-15000}" \
  --trainer.num_warmup_steps "${NUM_WARMUP_STEPS:-1000}" \
  --trainer.save_interval "${SAVE_INTERVAL:-2500}" \
