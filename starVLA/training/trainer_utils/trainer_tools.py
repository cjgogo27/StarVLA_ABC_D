...
class TrainerUtils:
    ...
    def load_pretrained_backbones(model, checkpoint_path=None, reload_modules=None):
        ...
        # Full VLM checkpoints are large; loading them on every rank at once can
        # spike CPU memory enough for the job to be SIGKILLed without traceback.
        for load_rank in range(world_size):
            ...
            if reload_modules:  # partial load
                ...
                for path in module_paths:
                    reload_module_names = path.split(".")
                    module = model
                    try:
                        for module_name in reload_module_names:  # find the module to modify level by level
    ...
    def setup_distributed_training(accelerator, *components):
        ...
        if dist.is_initialized() and dist.get_rank() == 0:
            logger.info("Calling accelerator.prepare(...)")
        prepared_components = accelerator.prepare(*components)
        if dist.is_initialized() and dist.get_rank() == 0:
            logger.info("accelerator.prepare(...) finished")
