...
def setup_optimizer_and_scheduler(model, cfg) -> Tuple[torch.optim.Optimizer, torch.optim.lr_scheduler._LRScheduler]:
    ...
    positive_lrs = [float(group["lr"]) for group in param_groups if float(group.get("lr", 0.0)) > 0.0]
    optimizer_default_lr = float(cfg.trainer.learning_rate.base)
    if optimizer_default_lr <= 0.0 and positive_lrs:
        optimizer_default_lr = max(positive_lrs)
    optimizer = torch.optim.AdamW(
        param_groups,
        lr=optimizer_default_lr,
    ...
    if dist.is_initialized() and dist.get_rank() == 0:
        logger.info(f"Optimizer default lr={optimizer.defaults['lr']}")
...
class VLATrainer(TrainerUtils):
    ...
    def _init_tensorboard(self):
        """Initialize TensorBoard scalar logging."""
        self.tb_writer = None
        if self.accelerator.is_main_process:
            tensorboard_dir = os.path.join(self.config.output_dir, "tensorboard")
            os.makedirs(tensorboard_dir, exist_ok=True)
            self.tb_writer = SummaryWriter(log_dir=tensorboard_dir)
            logger.info(f"TensorBoard logs will be written to {tensorboard_dir}")
    ...
    def _train_step(self, batch_vla, batch_vlm=None):
        ...
        with self.accelerator.accumulate(self.model):
            ...
            with torch.autocast("cuda", dtype=torch.bfloat16):
                ...
                aux_losses = {}
                loss_weights = output_dict.get("loss_weights", {})
                for loss_name, loss_value in output_dict.items():
                    if not loss_name.endswith("_loss") or loss_name == "action_loss":
                        continue
                    weight = float(loss_weights.get(loss_name, 1.0))
                    total_loss = total_loss + weight * loss_value
                    aux_losses[loss_name] = (loss_value, weight)
        ...
        for loss_name, (loss_value, weight) in aux_losses.items():
            metric_prefix = loss_name[:-5] if loss_name.endswith("_loss") else loss_name
            metrics[f"{metric_prefix}_loss"] = loss_value.item()
            metrics[f"{metric_prefix}_loss_weight"] = weight
        for metric_name, metric_value in output_dict.items():
            if metric_name in metrics or metric_name == "loss_weights" or metric_name.endswith("_loss"):
                continue
            if isinstance(metric_value, torch.Tensor):
                if metric_value.numel() == 1:
                    metrics[metric_name] = metric_value.detach().float().item()
