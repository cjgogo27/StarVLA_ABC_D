...
class LeRobotSingleDataset(Dataset):
    ...
    def _pack_sample(self, data: dict, trajectory_id: int | None = None, base_index: int | None = None) -> dict:
        ...
        for video_key in self.modality_keys["video"]:
            ...
            image = self._augment_sample_image(image)
        ...
        if self.data_cfg is not None and self.data_cfg.get("include_motion_history", False) not in ["False", False]:
            history_window = int(self.data_cfg.get("motion_history_window", 4))
            if self.data_cfg.get("include_action_history", True) not in ["False", False]:
                sample["action_history"] = action[:history_window].astype(np.float16)
            if "state" in sample:
                if trajectory_id is not None and base_index is not None:
                    past_state = []
                    for state_key in self.modality_keys["state"]:
                        past_state.append(
                            self.get_state_or_action_history(
                                trajectory_id,
                                "state",
                                state_key,
                                base_index,
                    ...
                    sample["state_history"] = np.concatenate(past_state, axis=1).astype(np.float16)
                else:
                    sample["state_history"] = sample["state"][-history_window:].astype(np.float16)

        if self._future_latent_alignment_enabled() and trajectory_id is not None and base_index is not None:
            sample["future_image"] = self._get_future_alignment_images(trajectory_id, base_index)

        return sample

    def get_state_or_action_history(
        self,
        trajectory_id: int,
        modality: str,
        key: str,
        ...
        *,
        ...
        start_offset = end_offset - history_window + 1
        step_indices = np.arange(start_offset, end_offset + 1, dtype=np.int64) + base_index
        ...
        return data[:, le_indices]

    def _future_latent_alignment_cfg(self):
        if self.data_cfg is None:
            return None
        return self.data_cfg.get("future_latent_alignment", None)

    def _future_latent_alignment_enabled(self) -> bool:
        flare_cfg = self._future_latent_alignment_cfg()
        return bool(flare_cfg and flare_cfg.get("enabled", False))

    def _get_future_alignment_images(self, trajectory_id: int, base_index: int) -> list[Image.Image]:
        """Load future observation frames used by optional FLARE-style losses."""
        flare_cfg = self._future_latent_alignment_cfg() or {}
        fallback_offset = int(self.data_cfg.get("action_horizon", 8)) if self.data_cfg is not None else 8
        future_offset = int(flare_cfg.get("future_offset", fallback_offset))
        augment_future = bool(flare_cfg.get("augment_future", False))

        future_images = []
        for video_key in self.modality_keys["video"]:
            frames = self.get_video(trajectory_id, video_key, base_index + future_offset)
            image = Image.fromarray(frames[0]).resize((224, 224))
            if augment_future:
                image = self._augment_sample_image(image)
            future_images.append(image)
        return future_images

    def _augment_sample_image(self, image: Image.Image) -> Image.Image:
        """Apply lightweight domain randomization after all camera views are resized."""
        if self.data_cfg is None:
            return image

        aug_cfg = self.data_cfg.get("visual_augmentation", None)
        if not aug_cfg or not aug_cfg.get("enabled", False):
            return image

        image = image.convert("RGB")

        crop_scale_min = float(aug_cfg.get("crop_scale_min", 1.0))
        ...
        def jitter(enhancer_cls, strength: float) -> None:
            nonlocal image
            if strength <= 0:
                return
        ...
        jitter(ImageEnhance.Brightness, float(aug_cfg.get("brightness", 0.0)))
        jitter(ImageEnhance.Contrast, float(aug_cfg.get("contrast", 0.0)))
        jitter(ImageEnhance.Color, float(aug_cfg.get("saturation", 0.0)))

        blur_prob = float(aug_cfg.get("blur_prob", 0.0))
        if blur_prob > 0 and random.random() < blur_prob:
            radius = random.uniform(0.1, float(aug_cfg.get("blur_radius", 0.6)))
            image = image.filter(ImageFilter.GaussianBlur(radius=radius))

        noise_prob = float(aug_cfg.get("noise_prob", 0.0))
        if noise_prob > 0 and random.random() < noise_prob:
            noise_std = float(aug_cfg.get("noise_std", 2.0))
            array = np.asarray(image).astype(np.float32)
            noise = np.random.normal(0.0, noise_std, array.shape).astype(np.float32)
            image = Image.fromarray(np.clip(array + noise, 0, 255).astype(np.uint8))

        jpeg_prob = float(aug_cfg.get("jpeg_prob", 0.0))
        if jpeg_prob > 0 and random.random() < jpeg_prob:
            quality_min = int(aug_cfg.get("jpeg_quality_min", 70))
            quality_max = int(aug_cfg.get("jpeg_quality_max", 95))
            quality = random.randint(max(1, quality_min), min(100, quality_max))
            buffer = io.BytesIO()
